...
### Registry Code Fallback Behavior

`PENDING` registryCode is an intentional offline fallback state.
It occurs when a preallocated sequence value is unavailable on the device.

This state does not indicate a persistence failure.
Sequence allocation continues normally when valid sequence data is available.
